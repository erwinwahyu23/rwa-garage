...
import SupplierPurchasesDialog from "./SupplierPurchasesDialog";

export default function SuppliersPageClient() {
  const router = useRouter();
  const [suppliers, setSuppliers] = useState<{ id: string; name: string; hasItems?: boolean }[]>([]);
  ...
  async function startEdit(s: { id: string; name: string }) {
    setEditingId(s.id);
  ...
  async function saveEdit(id: string) {
    ...
    setEditLoading(true);
    try {
      const res = await fetch(`/api/suppliers/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: editingName }) });
      if (!res.ok) throw new Error('Failed');
      const updated = await res.json();
      setSuppliers((s) => s.map((x) => x.id === id ? updated : x));
        ...
        {suppliers.map((s) => (
          <li key={s.id} className="py-1 flex items-center gap-4">
            {editingId === s.id ? (
              ...
              <SupplierPurchasesDialog supplier={s} />
              {s.hasItems ? <span className="text-sm text-gray-500 ml-2">(has items)</span> : null}
            </div>
                <div className="flex gap-2">
                  <Button variant="ghost" onClick={() => startEdit(s)}>Edit</Button>
                  <Button variant="destructive" onClick={() => deleteSupplier(s.id)} disabled={s.hasItems}>{s.hasItems ? 'In use' : 'Delete'}</Button>
                </div>
              </>
            )}
          </li>
